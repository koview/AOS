...
class ReviewsAdapter(private var reviews: List<MyReview>) : RecyclerView.Adapter<ReviewsAdapter.ReviewViewHolder>() {

    private lateinit var reviewImageAdapter: ReviewsImageAdapter

    interface MyItemClickListener{ //item clicklistner를 저장하기 위한 인터페이스
        // 삭제할 리뷰 선택 시작
    ...
    inner class ReviewViewHolder(private val binding: ItemReviewBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(review: MyReview) {
            ...
            binding.root.setBackgroundColor(
                if (review.isSelected) ContextCompat.getColor(context, R.color.kv_main7) else Color.WHITE
            )
